  ...
  var slider_plugin = (function() {
    var fifi_slider = function(settings) {
        ...
        _.def = {
          ...
          dotsWrapper: $('.header__container__dots'),
          transition: {
            speed: 300,
            easing: ''
          },
      ...
      fifi_slider.prototype.init = function () {
          ...
          function swipeEnd(e) {
            var touch = e;
            _.getCurLeft();  if (Math.abs(_.moveX - _.startX) === 0) return;  _.stayAtCur = Math.abs(_.moveX - _.startX) < 40 || typeof _.moveX === "undefined" ? true : false;
            _.dir = _.startX < _.moveX ? 'left' : 'right';  if (_.stayAtCur) {} else {
            ...
            delete _.startY;
            delete _.moveX;
            delete _.moveY;  _.isAnimating = false;
          ...
          function swipeMove(e) {
            ...
            if (Math.abs(_.moveX - _.startX) < 40) return;  _.isAnimating = true;
            addClass(_.def.target, 'isAnimating');
            e.preventDefault();  if (_.curLeft + _.moveX - _.startX > 0 && _.curLeft == 0) {
        ...
        _.curSlide = 0;
        _.curLeft = 0;
        _.totalSlides = _.def.target.querySelectorAll('.header__container__carousel--slide').length;
        _.sliderInner = _.def.target.querySelector('.header__container__carousel--wrapper');
        _.loadedCnt = 0;


        var cloneFirst = _.def.target.querySelectorAll('.header__container__carousel--slide')[0].cloneNode(true);
        _.sliderInner.appendChild(cloneFirst);
        var cloneLast = _.def.target.querySelectorAll('.header__container__carousel--slide')[_.totalSlides - 1].cloneNode(true);
        _.sliderInner.insertBefore(cloneLast, _.sliderInner.firstChild);

        _.curSlide++;
        _.allSlides = _.def.target.querySelectorAll('.header__container__carousel--slide');
      ...
      fifi_slider.prototype.gotoSlide = function () {
            ...
            _.sliderInner.style.left = -_.curSlide * _.slideW + 'px';  addClass(_.def.target, 'isAnimating');  
      ...
      fifi_slider.prototype.updateSliderDimension = function () {
        var _ = this;  _.slideW = parseInt(_.def.target.querySelectorAll('.header__container__carousel--slide')[0].offsetWidth);
        _.sliderInner.style.left = -_.slideW * _.curSlide + "px";  if (_.def.autoHeight) {
            _.def.target.style.height = _.allSlides[_.curSlide].offsetHeight + "px";
        } else {
